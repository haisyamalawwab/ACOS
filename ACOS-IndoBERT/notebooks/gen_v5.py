...
Dijalankan via: python gen_v5.py
"""
import json, os, sys
...
def code(src: str, tags=None) -> dict:
...
CELLS.append(md(
    ...
    "> Setiap step, epoch, dan run disimpan lengkap: log CSV, plot gambar, laporan Markdown.\n\n"
    "| Item | Nilai |\n"
    "|------|-------|\n"
    ...
    "| **Output per Run** | CSV log, 5 plot PNG, laporan Markdown, JSON, checkpoint |\n\n"
    "## Struktur Output per Run\n\n"
    "```\n"
    "experiments/<run_id>/\n"
    "  run_config.json          konfigurasi run ini\n"
    "  run_result.json          ringkasan metrik akhir\n"
    "  hardware_log.json        VRAM + waktu per epoch\n"
    "  step1/\n"
    "    training_log.csv       loss, F1, VRAM, waktu per epoch\n"
    "    eval_per_epoch.csv     TP, FP, FN, P, R, F1 per epoch\n"
    "    best_metrics.json      best epoch & F1\n"
    ...
    "      03_vram_usage.png\n"
    "      04_speed_curve.png\n"
    "      05_combined_dashboard.png\n"
    "    checkpoints/step1_best/\n"
    "  step2/  (struktur sama dengan step1)\n"
    "  inference/\n"
    "    test_predictions.tsv\n"
    "    test_sample_10.json\n"
    "    test_inference_summary.json\n"
    "  reports/\n"
    "    REPORT_INDEX.md\n"
    "    full_report.md\n"
    "```\n\n"
...
CELLS.append(code(
    ...
    "import torch\n"
    "\n"
    "# Sel 3: Import semua modul & setup path dua-root (Pola V4 Robust)\n"
    ...
    "\n"
    "# ============================================================\n"
    "# 1. DETEKSI ROOT DIREKTORI PROYEK (MENIRU V4 CELL 9 & 12)\n"
    "# ============================================================\n"
    ...
    "    for _c in kandidat:\n"
    "        p = Path(_c)\n"
    "        if p.is_dir() and ((p / 'Extract-Classify-ACOS').is_dir() or (p / 'ACOS-IndoBERT').is_dir()):\n"
    ...
    "\n"
    "# 2. Validasi & sinkronisasi modul acos_id (pola V4)\n"
    ...
    "]\n"
    "if _missing:\n"
    "    print(f'📥 Modul acos_id belum lengkap ({_missing}). Menyinkronkan...')\n"
    ...
    "        _tgt = os.path.join(_acos_id_dir, _fn)\n"
    "        try:\n"
    "            urllib.request.urlretrieve(_url, _tgt)\n"
    "            print(f'   ✅ {_fn} berhasil diunduh dari GitHub')\n"
    "        except Exception as _e:\n"
    "            print(f'   ✗ Gagal mengunduh {_url}: {_e}')\n"
    "\n"
    "# 3. Setup sys.path & bersihkan cache modul\n"
    "def _prepend_path(p):\n"
    ...
    "print(f'  ✓ taxonomy ({len(acos_taxonomy.CATEGORIES)} categories)')\n"
    "\n"
...
# ---------------------------------------------------------------------------
CELLS.append(md(
    "## Sel 8 — Training Adapter Function (dengan ResultSaver)\n\n"
    "Fungsi `train_one_run(cfg)` mengintegrasikan:\n"
    "- **`HardwareMonitor`** — mencatat VRAM, GPU util, kecepatan per epoch\n"
    "- **`ResultSaver`** — menyimpan CSV, PNG plot, laporan MD per step\n\n"
    "Setiap epoch langsung disimpan ke disk — tidak perlu menunggu selesai.\n\n"
...
CELLS.append(code(
    ...
    "# ResultSaver menyimpan SEMUA output per epoch ke folder terorganisasi\n"
    ...
    "\n"
    "    Output disimpan ke cfg['result_dir']:\n"
    "      step1/training_log.csv   log per epoch\n"
    "      step1/eval_per_epoch.csv TP/FP/FN per epoch\n"
    "      step1/best_metrics.json  best epoch & F1\n"
    "      step1/plots/*.png        5 plot gambar\n"
    "      step2/  (sama)\n"
    "      inference/*.tsv/.json   prediksi test set\n"
    "      reports/full_report.md  laporan narasi\n"
    "      run_result.json         ringkasan akhir\n"
    "    \"\"\"\n"
    "    run_id  = cfg['run_id']\n"
    "    num_ep  = cfg['epochs']\n"
    ...
    "\n"
    "    # ============================================================\n"
    "    # ResultSaver — menyimpan semua output per step\n"
    "    # ============================================================\n"
...
notebook = {
    ...
    "metadata": {
        ...
        "accelerator": "GPU",
        "notebook_version": "V5-MI300X-2026-09-24",
        "description": (
            "ACOS IndoBERT V5 Full Experiment Pipeline — AMD MI300X edition. "
            "No early stopping. 54 run otomatis (multi-epoch x multi-split x K-Fold CV). "
            "Hardware monitoring lengkap (VRAM, waktu, GPU utilization)."
        ),
